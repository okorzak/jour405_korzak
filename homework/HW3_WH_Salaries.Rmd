...
The data displays all of the salaries of white house employees. Most range around the $100,000
...
wh_salaries|>
  summarize(median(salary),na.rm=TRUE)
```
...
wh_salaries|>
  summarize(median(salary),na.rm=TRUE)
wh_salaries|>
  summarize(min_salary = min(salary), na.rm=TRUE)
wh_salaries|>
...
I have no idea what I'm doing wrong. I am following what we did in class and looking stuff up online and I can't figure out why I keep getting errors with the median and the NA. I can tell you that the high ends of the 100,000s. The min is 0.
...
A potential story idea with this data is exploring which positions in the white house make the most / the least among all of the white house positions.  You could also do a deep dive into what each position consists of.
