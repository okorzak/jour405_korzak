...
It appears that income, education level, and median age work in correlation with turnout, as there are clear positive relationships. Population and unemployment show little correlation and negative relationships with voter turnout in the scatterplots. Overall, education, income, and age appear to have positive relationships with turnout.
...
The model shows that income, education, and median age are significant predictors of turnout, while population and unemployment are insignificant. The model fits reasonably well, explaining a good portion of turnout variation. I would say that it’s a useful model, but it does include some variables that don't add much.
...
Model 3 worked practically as well as the original. I removed employment status and added the percent of whites to model 2 and it ended up coming out very strong. Model 2 also performed well, around 75% of what model 3 and the original performed. Model 4 was essentially useless, the adjusted r-square value was less than 25% of what the other models were.
...
This analysis can help show how counties vote at higher or lower rates based on certain variables. A journalist could use the data from the models to explain  patterns or highlight variables that prevent counties from voting. It provides context for any election or voter participation story.